...
<body>
			...
			<ul class="navbar-nav ml-auto">
				...
				<li class="nav-item">
					<form method="post">
						...
						if (empty($cust_id)) {
							?>
							<span style="color:black; font-size:30px;"><i class="fa fa-shopping-cart"
									aria-hidden="true"><span style="color:red;" id="cart"
										class="badge badge-light">4</span></i></span>
							...
							?>
							<a href="cart.php"><span style="color:black; font-size:30px;"><i class="fa fa-shopping-cart"
										aria-hidden="true"><span style="color:black;" id="cart" class="badge badge-light">
											<?php if (isset($re))
												echo $re; ?>
										</span></i></span></a>
	...
	<br><br>
	<div class="middle" style="  padding:60px; border:1px solid #000000;  width:100%;">
		<!--tab heading-->
		<ul class="nav nav-tabs nabbar_inverse" id="myTab" style="background:#000000;border-radius:10px 10px 10px 10px;"
			role="tablist">
			<li class="nav-item">
				<a class="nav-link active" style="color:#00ffff;" id="viewitem-tab" data-toggle="tab" href="#viewitem"
					role="tab" aria-controls="viewitem" aria-selected="true">View Cart</a>
			</li>
			<li class="nav-item">
			...
			<li class="nav-item">
				<a class="nav-link" style="color:#00ffff;" id="orders-tab" data-toggle="tab" href="#orders" role="tab"
					...
					<tbody>
						...
						if ($re) {
							while ($res = mysqli_fetch_array($query)) {
								...
								<tr>
									<td>
										<image src="../image/merchant/<?php echo $em . "/goodimages/" . $res['fldimage']; ?>"
									...
									<form method="post" enctype="multipart/form-data">
										<td><button type="submit" name="del" value="<?php echo $res['fld_cart_id'] ?>"
												class="btn btn-danger">Remove</button></td>
									</form>
									<td>
										<?php $total = $total + $res['cost'];
										$gtotal[] = $total; ?>
									</td>
								</tr>
							...
							<tr>
								...
								<td style="padding:30px; text-align:center;">
									<a href="order.php?cust_id=<?php echo $cust_id; ?>"><button type="button"
											style=" color:white; font-weight:bold; text-transform:uppercase;"
											class="btn btn-warning">Proceed to checkout</button></a>
								</td>
								<td></td>
							</tr>
							...
							?>
							<tr><button type="button" class="btn btn-outline-success"><a href="../index.php"
										style="color:green; text-decoration:none;">Your order has been placed Shop more!
									</a></button></tr>
				...
				<form method="post" enctype="multipart/form-data">
					...
					<div class="form-group">
						...
						<input type="tel" id="mobile" class="form-control" name="mobile"
							pattern="[6-9]{1}[0-9]{2}[0-9]{3}[0-9]{4}" value="<?php if (isset($fld_mobile)) {
								echo $fld_mobile;
				...
				<table class="table">
					...
					<tbody>
						...
						while ($roww = mysqli_fetch_array($quer_res)) {
							$fid = $roww['fld_good_id'];
							$qr = mysqli_query($con, "select * from tbgood where good_id='$fid'");
							...
							<tr>
								<td>
									<?php echo $roww['fld_order_id']; ?>
								</td>
								<?php
								if (empty($qrr['goodname'])) {
									?>
									<td><span style="color:red;">Product is Not Available Now</span></td>
									<?php
								} else {
									?>
									<td>
										<?php echo $qrr['goodname']; ?>
									</td>
									<?php
								}
								?>
								...
								<td><a href="#" onclick="del(<?php echo $roww['fld_order_id']; ?>);"><button type="button"
