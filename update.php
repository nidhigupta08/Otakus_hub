...
if(!empty($_GET['good_id']))
{
	...
	$query=mysqli_query($con,"select * from tbgood   where good_id='$good_id'");
if(mysqli_num_rows($query))
{   
	 $row=mysqli_fetch_array($query);
     $rgoodname=$row['goodname'];
     $rcost=$row['cost'];
     $rcategories=$row['categories'];
   ...
   if(!empty($_SESSION['id']))	
   {
    ...
    if(!empty($chk)) 
	{
		if(empty($img_name))
	       ...
	       {
		          $paymentmode=implode(",",$chk);
	              if(mysqli_query($con,"update  tbgood  set goodname='$good_name',cost='$cost',categories='$categories',paymentmode='$paymentmode' where good_id='$good_id'"))
	   
	                {
						header("location:update_good.php?good_id=$good_id");
		              //echo "update with old pic";
		              //move_uploaded_file($_FILES['good_pic']['tmp_name'],"../image/merchant/$em/goodimages/".$_FILES['good_pic']['name']);
	                 }
	              else{
		               echo "failed";
	                  }
	     ...
	     else
		 {
			     ...
			     echo $paymentmode."<br>";
			     echo $img_name."<br>";
	             if(mysqli_query($con,"update  tbgood  set goodname='$good_name',cost='$cost',categories='$categories',paymentmode='$paymentmode', fldimage='$img_name' where good_id='$good_id'"))
	
	                {
		             echo "update with new pic";
		             move_uploaded_file($_FILES['good_pic']['tmp_name'],"image/merchant/$em/goodimages/".$_FILES['good_pic']['name']);
	                 unlink("image/merchant/$em/goodimages/$rfldimageold");
					 header("location:update_good.php?good_id=$good_id");
					}
				 else
				 {
                  ...
                  <!--add Product-->
                        <form action="" method="post" enctype="multipart/form-data">
                                     ...
                                     <label for="good_name">Product Name:</label>
                                            <input type="text" class="form-control" id="good_name" value="<?php if(isset($rgoodname)) { echo $rgoodname;}?>" placeholder="Enter product Name" name="good_name" required>
                                     </div>
	                                 ...
	                                 <div class="form-group"><!--categories-->
                                            ...
                                            <input type="text" class="form-control" id="categories" value="<?php if(isset($rcategories)) { echo $rcategories;}?>" placeholder="Enter categories" name="categories" required>
                                    </div>
	                                ...
	                                <div class="form-group">
                                         ...
                                         <input type="file" accept="image/*" name="good_pic"/>merch Snaps 
                                    </div>
