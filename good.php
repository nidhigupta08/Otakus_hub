...
{   if(!file_exists("image/merchant/".$id."/goodimages"))
	{
		$dir=mkdir("image/merchant/".$id."/goodimages");
	}
...
{   if(isset($_SESSION['id']))
	{
    $img_name=$_FILES['good_pic']['name'];
    if(!empty($chk)) 
	{
	$paymentmode=implode(",",$chk);
	if(mysqli_query($con,"insert into tbgood(fldvendor_id,goodname,cost,categories,paymentmode,fldimage) values
	
	('$v_id','$good_name','$cost','$categories','$paymentmode','$img_name')"))
	{
		move_uploaded_file($_FILES['good_pic']['tmp_name'],"image/merchant/$id/goodimages/".$_FILES['good_pic']['name']);
	}
	else{
		echo "failed";
	}
...
if(isset($upd_account))
			  ...
			  if(isset($upd_logo))
			  {
				  ...
				  {
					  unlink("image/merchant/$id/$old_logo");
					  move_uploaded_file($_FILES['logo_pic']['tmp_name'],"image/merchant/$id/".$_FILES['logo_pic']['name']);
				      
					  header("location:update_good.php");
...
<br><br>
<div class="middle" style=" padding:40px; border:1px solid #000000;  width:100%;background-color:azure">
       <!--tab heading-->
	   ...
	   <div class="tab-content" id="myTabContent">
            ...
            <div class="tab-pane fade show active" id="viewitem" role="tabpanel" aria-labelledby="home-tab">
			 ...
			 <th>Update item Details </th>
			   <?php
					  if($query=mysqli_query($con,"select tblvendor.fldvendor_id,tblvendor.fld_email,tbgood.good_id,tbgood.goodname,tbgood.cost,tbgood.paymentmode,tbgood.categories,tbgood.fldimage,tblvendor.fld_name from tblvendor inner join tbgood on tblvendor.fldvendor_id=tbgood.fldvendor_id where tblvendor.fld_email='$id'"))
					  {
						  if(mysqli_num_rows($query))
						  {
				...
				<td><img src="<?php echo 'image/merchant/'.$id.'/goodimages/'.$row['fldimage'];?>" height="100px" width="150px"></td>
				<td style="width:150px;"><?php  echo $row['goodname']."<br>";?></td>
				<td align="center" style="width:150px;"><?php  echo $row['cost']."<br>";?></td>
				<td  align="center" style="width:150px;"><?php  echo $row['categories']."<br>";?></td>
				<td align="center" style="width:150px;"><?php  echo $row['paymentmode']."<br>";?></td>
				
				<td align="center" style="width:150px;">
				
				<a href="vendor_delete_good.php?good_id=<?php echo $row['good_id'];?>"><button type="button" class="btn btn-warning">Delete </button></a>
				
				</td>
				<td align="center" style="width:150px;">
				
				<a href="update.php?good_id=<?php echo $row['good_id'];?>"><button type="button" class="btn btn-danger">Update </button></a>
            ...
            <div class="tab-pane fade" id="manageaccount" role="tabpanel" aria-labelledby="profile-tab">
			         <!--add Product-->
                        <form action="" method="post" enctype="multipart/form-data">
                                     <div class="form-group"><!--good_name-->
                                     <label for="good_name">Product Name:</label>
                                            <input type="text" class="form-control" id="good_name" value="<?php if(isset($good_name)) { echo $good_name;}?>" placeholder="Enter Product Name" name="good_name" required>
                                     </div>
	                                 ...
	                                 <div class="form-group"><!--categories-->
                                            ...
                                            <input type="text" class="form-control" id="categories" value="<?php if(isset($categories)) { echo $categories;}?>" placeholder="Enter Categories" name="categories" required>
                                    </div>
	                                ...
	                                <div class="form-group">
                                         <input type="file" accept="image/*" name="good_pic" required/>Merch Snaps 
                                    </div>
			...
			<!--tab 2 ends-->
			 ...
			 <div class="tab-pane fade" id="accountsettings" role="tabpanel" aria-labelledby="accountsettings-tab">
				...
				?>
                  ...
                  <button type="submit" name="upd_account" style="background:#000000; border:1px solid #00ffff;" class="btn btn-primary">Update</button>
			 ...
			 <div class="tab-pane fade " id="status" role="tabpanel" aria-labelledby="status-tab">
				...
				if(mysqli_num_rows($orderquery))
				{
					while($orderrow=mysqli_fetch_array($orderquery))
					{
						...
						<td><?php echo $orderrow['fld_good_id']; ?></td>
						<?php
